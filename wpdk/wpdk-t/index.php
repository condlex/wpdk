...
if ( $phase == 1 || $phase == 5 ) {
  ...
  $maximum_batch_no = intval( $_GET['bm'] );
  if ( $maximum_batch_no <= 0 ) {
    $maximum_batch_no = 1;
  }
...
EOT;
  ...
  for ( $id = $from_count; $id <= $to_count; $id++ ) {
    ...
    wpdk_save_post( $post );
    $time_end_i = microtime( true );
    $time_list[] = ( $time_end_i - $time_start_i );
    if ( $id == $from_count ) {
      $sample_size = strlen( wpdk_serialize( $post ) );
    }
  }
  $time_end_a = microtime( true );
  $time_a = ( $time_end_a - $time_start_a ) * 1000 * 1000;
  $time_max = max( $time_list ) * 1000 * 1000;
  $time_min = min( $time_list ) * 1000 * 1000;
...
if ( $phase == 2 ) {
  wpdk_delete_all_posts();
  header( 'Content-Type: text/plain' );
  require_once __DIR__ . '/wpdk/config.php';
  $directory = $wpdk_config['data_dir'] . '/posts';
  $is_dir = is_dir( $directory ) ? '1' : '0';
  echo '[C] ', $is_dir, '\n';
}

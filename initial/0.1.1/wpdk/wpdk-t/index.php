...
$g_script_file = $wpdk_config['build_uri'];
...
EOT;
    ...
    $tt_il[] = ($te_i - $ts_i);
    if ($id == $from_cnt) {
      $s_size = strlen(serialize($post));
    }
  }
  $te_a = microtime(true);
  $tt_a = ($te_a - $ts_a) * 1000 * 1000;
  $tt_imx = max($tt_il) * 1000 * 1000;
  $tt_imn = min($tt_il) * 1000 * 1000;

  if ($phase == 1) {
    header('Content-Type: text/plain');
    echo "[C] ", $bn, ", ", $bs, ' --> ', $from_cnt, ', ', $to_cnt, ' --> ', $s_size, ' --> ', wpdk_last_post_id(), ' [Time] ', 'Total: ', $tt_a, ', Min Item: ', $tt_imn, ', Max Item: ', $tt_imx, "\n";
  } else {
    if ($bn >= $bm) {
      header('Content-Type: text/plain');
      echo "[C] ", $bn, ", ", $bs, ' --> ', $from_cnt, ', ', $to_cnt, ' --> ', $s_size, ' --> ', wpdk_last_post_id(), ' [Time] ', 'Total: ', $tt_a, ', Min Item: ', $tt_imn, ', Max Item: ', $tt_imx, "\n";
      ...
      $url = $g_script_file . '?bs=' . $bs . '&bn=' . ($bn+1) . '&bm=' . $bm . '&ph=5';
...
if ($phase == 2) {
  ...
  $dir = $wpdk_config['data_dir'] . '/posts';
  $b_dir = is_dir($dir) ? '1' : '0';
  echo "[C] ", $b_dir, "\n";
}
